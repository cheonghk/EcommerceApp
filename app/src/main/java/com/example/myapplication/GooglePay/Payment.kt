    ...
    fun executeTransaction(googlePayData: String, activity: Activity) {
        doAsync {
            try {
                ...
                val executeTransactionPayload = JSONObject()
                ...
                executeTransactionPayload.put("createPaymentData", payload)

                // Payload wallet part
                executeTransactionPayload.put("walletPayload", JSONObject(googlePayData))
                ...
                writer.write( executeTransactionPayload.toString())
                writer.flush()
                writer.close()
                os.close()
                    ...
                    activity.runOnUiThread {
                        try {
                            ...
                            if (transactionStatus == "AUTHORISED") {
                                ...
                                returnsResult(false, PaymentErrorCode.PAYMENT_REFUSED_ERROR, "Payment refused", activity)
                            }
                ...
                activity.runOnUiThread {
                    returnsResult(false, PaymentErrorCode.TIMEOUT_ERROR, "", activity)
                }
            } catch (e: ErrnoException) {
                activity.runOnUiThread {
                    returnsResult(false, PaymentErrorCode.NO_CONNECTION_ERROR, "", activity)
                }
            } catch (e: IOException) {
                activity.runOnUiThread {
